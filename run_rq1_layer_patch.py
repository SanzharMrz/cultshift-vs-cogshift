...
def main():
    ...
    # Measure effects: refusal delta via short masked generation (patched start)
    ...
    def short_generation_refusal_from_logits(first_step_logits, model, tok, text, allowed_ids, max_new_tokens=8) -> int:
        # Use provided first-step logits (from patched forward) as the first token, then continue
        with torch.no_grad():
    ...
    for L in layers:
        ...
        for text in tqdm(kept, desc=f"Layer {L} patching", unit="prompt"):
            ...
            # score patched vs original using short generation starting from patched first step
            with torch.no_grad():
                enc_t = tok_t(format_chat(tok_t, text), return_tensors="pt").to(device)
                out_t = tuned(**enc_t)
            if args.debug:
                # capture unpatched residual to compare
                h_t_orig = collect_last_token_resids(tuned, tok_t, [text], L, device=device)[0]  # CPU float32
            ...
            # KL next-token at last content index (masked, allowed subset only)
            ...
            with torch.no_grad():
                ...
                out_b = base(**enc_b)
            if args.debug:
                h_b_orig = collect_last_token_resids(base, tok_b, [text], L, device=device)[0]  # CPU float32
