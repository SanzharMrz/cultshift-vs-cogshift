...
def main():
    ...
    # Include hook in filename to avoid overwriting across hooks for the same layer
    suffix = f"_{args.split}" if args.split in ("train","val") else ""
    out_name = f"mapped_patch_L{layer}_{hook}{suffix}.json"
    with open(os.path.join(out_dir, out_name), "w", encoding="utf-8") as f:
        json.dump(res, f, ensure_ascii=False, indent=2)
    print("Saved mapped_patch results (split=", args.split, "):", os.path.join(out_dir, out_name))


if __name__ == "__main__":
    main()

from mechdiff.experiments.cultural.rq2.run_rq2_mapped_patch import *  # noqa: F401,F403
